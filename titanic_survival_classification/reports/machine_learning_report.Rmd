...
1. **Data Cleaning** - As mentioned in the Exploratory Analysis, there are some Null and NaN value that needs to be taken care of.
...
Recall that a Random Forest is a collection of decision trees, each tree being built a bootstrapped data set. At each split within any given tree, only a random sample of the predictors are available as candidates on which to create the split. This decorrelates the trees so that when multiple learners are averaged together, variance is reduced and the algorithm generalizes well.

A Decision Forest is slightly different in that each decision tree within the ensemble **only has access to a random subset of the predictors.** Every node within a tree is then allowed to consider the full feature space **withing the already filtered feature space for that tree.** For example, if one is working with a **n x 12** data set, each tree within a Decision Forest will utilized all *n* observations, but only a random subset of the 12 features.
