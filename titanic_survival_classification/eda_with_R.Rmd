...
plt <- plt + labs(x = 'Age', y = 'Density', title = 'Age Histogram')
...
a <- a + labs(x="", y = "Percentage",
              title = "Gender Probabilities Conditioned on Survival Status")
a <- a + geom_text(aes(
...
parch_df <- with(training, table(Survived, Parch))
knitr::kable(parch_df, caption = "Death & Surival Counts by Number of Parents or Children Aboard")
```


With regard to the previous paragraph, note that the sample sizes per possible value in the $SibSp$ column decrease dramatically after 2. In the same vein as above, making assumptions about the population (test set) using $SibSp$ with values greater than two would be statistically irresponsible.
...
sibsp_df <- with(training, table(Survived, SibSp))
knitr::kable(sibsp_df, caption = "Death & Surival Counts by Number of Parents or Children Aboard")
```
